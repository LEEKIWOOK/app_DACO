    ...
    try {
        ...
        fastaToJson(data, function (result) {
            ...
            if (result[0]['messages'].length == 0) {
                ...
                alert('Oops... something wrong. Please input correct Fasta format file.')
                throw new Error("Oops... something wrong. Please upload correct Fasta file.");
            }
        });
    } catch (error) {
        event.sender.send('gb_err')
    ...
    try {
        fastaToJson(data, function (result) {
            ...
            if (result[0]['messages'].length == 0) {
                ...
                // fs.writeFile('cds.fa', '>cds\n' + tem_cds, 'utf8');
                // cds = result[0]['parsedSequence']['sequence'].toUpperCase();
            } else {
                alert('Oops... something wrong. Please input correct Fasta format file.')
                throw new Error("Oops... something wrong. Please upload correct Fasta file.");
            }
...
ipcMain.on('analyse', function (event, data) {
    ...
    output_info.push(['Cas9 variants', data['Cas9 variants']])
    output_info.push(['Deaminase', data['Deaminase']])
    output_info.push(['PAM', data['pam']])
    output_info.push(['gRNA length', data['grna_length']])
    output_info.push(['Edit window', data['Edit window']])
    ...
    if (parseInt(data.Edit_window_start) < 1 | parseInt(data.Edit_window_end) > parseInt(data.grna_length)) {

        throw new Error("Oops..., Edit window should be 1 to gRNA length range, please reset the parameters.");
    }


    if (typeof (data.pam) !== 'string' | isNaN(data.Edit_window_start) | isNaN(data.Edit_window_end) | isNaN(data.grna_length) | data.Edit_window_start == 0 | data.Edit_window_end == 0 | data.grna_length == 0) {
        
        throw new Error("The edit system option may be error, please check.");
    }
    ...
    if (data.FA) {
        ...
        if ( tem_cds.substring(0, 3) !== 'ATG' & data.cds_sequence.substring(0, 3) !== 'ATG') {
            throw new Error("The CDS sequence does not start with ATG, please check.");
        }
