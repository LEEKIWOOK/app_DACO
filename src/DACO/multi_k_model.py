...
class SingleKModel:
    def __init__(self, model):
        ...
        self.vocab_lst = sorted(list(model.index_to_key))

class MultiKModel:
    def __init__(self, filepath):
        #self.aggregate = word2vec.Word2Vec.load_word2vec_format(filepath, binary=False)
        self.aggregate = gensim.models.keyedvectors.KeyedVectors.load_word2vec_format(filepath, binary=False)
        
        #vocab_lens = [len(vocab) for vocab in self.aggregate.vocab.keys()]
        vocab_lens = [len(x) for x in list(self.aggregate.index_to_key)]
    ...
    def separate_out_model(self, k_len):
        ...
        vocabs = [vocab for vocab in list(self.aggregate.index_to_key) if len(vocab) == k_len]
        ...
        with tempfile.NamedTemporaryFile(mode='w') as fptr:
            ...
            fptr.flush()
            return SingleKModel(gensim.models.keyedvectors.KeyedVectors.load_word2vec_format(fptr.name, binary=False))