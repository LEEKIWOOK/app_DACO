...
function savefile(all_info) {
    ...
    var excel_info = new Array()

    excel_info[0] = ['Edit System option']
    excel_info[1] = ['gene DNA sequence', '']
    excel_info[2] = ['gene CDS position']
    excel_info[3] = ['Note: The next information is the target codon and its designed gRNA.']



    for (var i in all_info) {

        if (all_info[i][0] === "Cas9 variants") {
            if (all_info[i][1] !== "") {
                excel_info[0].push("Cas9 variants:"+ all_info[i][1])
            }
        }

        if (all_info[i][0] === "Deaminase") {
            if (all_info[i][1] !== "") {
                excel_info[0].push("Deaminase:"+ all_info[i][1])
            }
        }

        if (all_info[i][0] === "PAM") {
            if (all_info[i][1] !== "") {
                excel_info[0].push("PAM:"+ all_info[i][1])
            }
        }

        if (all_info[i][0] === "gRNA length") {
            if (all_info[i][1] !== "") {
                excel_info[0].push("gRNA length:"+ all_info[i][1])
            }
        }

        if (all_info[i][0] === "Edit window") {
            if (all_info[i][1] !== "") {
                excel_info[0].push("Edit window:"+ all_info[i][1])
            }
        }


        if (all_info[i][0] === "DNA") {
            if (all_info[i].length == 5) {
                excel_info[1][1] += all_info[i][3]
            }
        }
        if (all_info[i][0] === "CDS") {
            excel_info[2].push(all_info[i][3] + ':' + (parseInt(all_info[i][1]) + 1) + "-" + all_info[i][2])
        }
        if (all_info[i][0] === "target codon") {

            tareget_codon_id = parseInt(all_info[i][5].substr(1))
            excel_info[3 + tareget_codon_id] = ["target codon:" + all_info[i][3] + "; " + "target codon ID:" + all_info[i][5] + "; " + "position in DNA:" + all_info[i][1] + "-" + all_info[i][2]]
        }

        if (all_info[i][0] === 'sgRNA') {

            tareget_codon_id = parseInt(all_info[i][7][3].substr(1))
            ...
            try {
                excel_info[3 + tareget_codon_id].push("position in DNA:" + all_info[i][1] + "-" + all_info[i][2] + "; " + "sequence:" + all_info[i][3] + "; " + "PAM:" + all_info[i][8] + "; " + "strand:" +
                    all_info[i][5] + "; " + "target_codon:" + all_info[i][7][3] + "; " + "Cytosine to be edited in gRNA position:" + all_info[i][6] + "; " + "offtarget_info:" + offtarget_info)
            } catch (e) {
                continue
            }
...
// function savefile(all_info){
//     var DNA = "", cDNA="", Codon="", protein="", gRNA="";
//     for (var i in all_info){
...
module.exports.sheet2blob = sheet2blob;
module.exports.openDownloadXLSXDialog = openDownloadXLSXDialog;
