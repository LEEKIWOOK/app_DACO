...
$(document).ready(function(){
    ...
    ipcRenderer.on('send_data', function (event, data) {
        ...
        if (info.category.length === 3) {
            alert('No suitable editing sites and gRNA were found! \n Warning: \n 1. Please make sure that the DNA and CDS are the same gene. \n 2. Make sure that the DNA and CDS sequences are on the same strand (5\'-3\'). \n 3. There is no suitable editing site for this gene.')
        }
    ...
    toolbox: {
        ...
        feature: {
            saveAsImage: {
                ...
                icon:'image://statics/img/photo.svg',
                pixelRatio:2
            },
            myTool1: {
                show: true,
                title: 'save result as excel',
                icon: 'image://statics/img/excel.svg',
                onclick: function () {
                    ...
                    //     })
                        ...
                        // fs.writeFile(result, func.savefile(output_info), (err) => {
                        //     if (err) {
                        //         alert('an error ocurred with file creation ' + err.message);
                        //         return
                        //     }
                        ...
                        excel_info = func.savefile(output_info)

                        var sheet = xlsx.utils.aoa_to_sheet(excel_info);
                    

                        var blob = func.sheet2blob(sheet);

                        func.openDownloadXLSXDialog(blob, 'CRISPR-BETS result.xlsx');


                        // fs.writeFile(result, blob, (err) => {
                        //     if (err) {
                        //         alert('an error ocurred with file creation ' + err.message);
                        //         return
                        //     }
...
Chart_gene.on('dataZoom', function (para) {
    ...
    if (para.hasOwnProperty('batch')) {
        ...
        if ((para['end'] - para['start']) * info.dna_len / 100 < 120 && i === 0) {
            option["series"][0]["label"]['normal']['show'] = true;
            option["dataZoom"][0]["start"] = para['start'];
            option["dataZoom"][0]["end"] = para['end'];
            Chart_gene.setOption(option, true);
            i += 1
        }
        if ((para['end'] - para['start']) * info.dna_len / 100 > 150 && i === 1) {
            option["series"][0]["label"]['normal']['show'] = false;
            option["dataZoom"][0]["start"] = para['start'];
            option["dataZoom"][0]["end"] = para['end'];
            Chart_gene.setOption(option, true);
